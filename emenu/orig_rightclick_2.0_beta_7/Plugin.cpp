...
int CPlugin::DialogEx(int X1, int Y1, int X2, int Y2, LPCTSTR szHelpTopic
		...
		, FARWINDOWPROC DlgProc, long pParam)
{
...
int CPlugin::AdvControl(int nCommand, void *pParam)
{
	return PluginStartupInfo::AdvControl(ModuleNumber, nCommand, pParam);
}

long WINAPI CPlugin::CfgDlgProcStatic(HANDLE hDlg, int Msg, int Param1, long Param2)
{
...
void CPlugin::CfgDlgProc(HANDLE hDlg, int Msg, int Param1, long Param2)
{
	if (DN_INITDIALOG==Msg)
	{
...
//	/*  */	{DI_TEXT, 0, nHeight-4, 0, 0, 0, 0, DIF_BOXCOLOR|DIF_SEPARATOR, 0, {0}},
	...
	if (14!=DialogEx(-1, -1, nWidth, nHeight, g_szTopicConfig, DlgItems
		, BOUNDS(DlgItems), 0, 0, CfgDlgProcStatic, reinterpret_cast<long>(this)))
	{
		return 0;
	}
...
HANDLE CPlugin::OpenPlugin(int nOpenFrom, int nItem)
{
...
CPlugin::EDoMenu CPlugin::OpenPluginBkg(int nOpenFrom, int nItem)
{

...
void MkFileInfo( char *buff,int bsz,CONSTSTR title,WIN32_FIND_DATA* p )
  {  char       str[ 100 ];
     SYSTEMTIME tm;
...
overCode FTP::AskOverwrite( int title, BOOL Download,WIN32_FIND_DATA* dest,WIN32_FIND_DATA* src,overCode last )
  {
    if ( !hConnect )
      return ocCancel;
...
//Set values
     //Title
     InitItems[ 0].Text = reinterpret_cast<const char*>(static_cast<INT_PTR>(title));

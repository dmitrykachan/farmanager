...
//---------------------------------------------------------------------------------
BOOL DECLSPEC AskSaveList( SaveListInfo* sli )
{
...
//Set flags
     ...
     StrCpy( DialogEditBuffer, Unicode::utf16ToUtf8(ci->DestPath).c_str(), DIALOG_EDIT_SIZE );
...
//Get paras
     //Path
     ci->DestPath = Unicode::utf8ToUtf16(DialogEditBuffer);
     if ( !ci->DestPath[0] )
       return FALSE;

     if ( ci->DestPath == FTP_CMDPREFIX &&
          ci->DestPath[FTP_CMDPREFIX_SIZE] == L':' )
...
{  
	...
	Host.MkUrl( str, path, L"", true);

	if(h.SetHostName(str, L"", L"") &&
...
#define dCONNECT 26
  ...
  Utils::safe_strcpy(di[dHOST].Data, Unicode::toOem(p->hostname_).c_str());
  Utils::safe_strcpy(di[dUSER].Data, Unicode::toOem(p->username_).c_str());
  Utils::safe_strcpy(di[dPSWD].Data, Unicode::toOem(p->password_).c_str());
  Utils::safe_strcpy(di[dDESC].Data, Unicode::toOem(p->hostDescription_).c_str());
  ...
  Utils::safe_strcpy(di[dLISTCMD].Data,	Unicode::toOem(p->listCMD_));
  ...
  p->SetHostName(Unicode::fromOem(Name), 
                  Unicode::fromOem(di[dUSER].Data),
                  Unicode::fromOem(di[dPSWD].Data));
  p->hostDescription_ = Unicode::fromOem(di[dDESC].Data);
  ...
  p->listCMD_ = Unicode::fromOem(di[dLISTCMD].Data);
...
//---------------------------------------------------------------------------------
bool FTP::EditDirectory(std::wstring& Name, std::wstring &Desc, BOOL newDir)
{
static FP_DECL_DIALOG( InitItems )
...
FP_END_DIALOG
   ...
   //Name
	 ...
	 StrCpy( DialogEditBuffer, Unicode::toOem(Name).c_str(), DIALOG_EDIT_SIZE );
   ...
   //Description
     if(!Desc.empty())
		 Utils::safe_strcpy(di[4].Data, Unicode::toOem(Desc));
      else
	  {
   ...
   //Fields -> Data
	 Name = Unicode::fromOem(DialogEditBuffer);
     if(!Desc.empty()) 
		 Desc = Unicode::fromOem(di[4].Data);
...
UINT FTP::SelectTable(UINT codePage)
{
	...
	typedef boost::array<CodePage, 5> CodePageArray;
	...
	CodePageArray::const_iterator itr = tables.begin();

	int i = 0;
	int index = 0;
	while(itr != tables.end())
	{
		Utils::safe_strcpy(MenuItems[i].Text, itr->name_);
		if(codePage == itr->id_)
		{
		...
		MenuItems[i].Separator = MenuItems[i].Checked = false;
		++itr; ++i;
	}

	int exitCode = FP_Info->Menu(FP_Info->ModuleNumber, -1, -1, 0, FMENU_AUTOHIGHLIGHT,
		FP_GetMsg(MTableTitle), NULL, NULL, NULL, NULL, MenuItems, static_cast<int>(tables.size()));

	if(exitCode < 0)
		return codePage;

	return tables[exitCode].id_;
}
...
FP_END_DIALOG
  ...
  {
	  m = Unicode::utf16ToUtf8(FTP_FILENAME( &PInfo.SelectedItems[n] )).c_str();
...
//---------------------------------------------------------------------------------
bool DECLSPEC GetLoginData(std::wstring &username, std::wstring &password, bool forceAsk)
{
	static FP_DECL_DIALOG( InitItems )
...
FP_END_DIALOG
	...
	Utils::safe_strcpy(di[2].Data, WinAPI::toOem(username));
	Utils::safe_strcpy(di[4].Data, WinAPI::toOem(password));
    ...
    do{
	  ...
	  username = WinAPI::fromOEM(di[2].Data);
      password = WinAPI::fromOEM(di[4].Data);
      return TRUE;

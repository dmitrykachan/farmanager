...
bool GetPDataNT(ProcessDataNT& DATA, ProcessPerfData& pd)
{
    ...
    pFullPath += 4;
    strncpy(DATA.FullPath, pFullPath, sizeof DATA.FullPath-1);
    strncpy(DATA.CommandLine, pd.CommandLine, sizeof DATA.CommandLine-1);
    return true;
}
...
//    Lock l(&Thread); // it's already locked in Plist::GetFindData
    ...
    for (DWORD i=0; i<numTasks; i++)
    {
      ...
      strncpy(CurItem.FindData.cFileName,pd.ProcessName,sizeof CurItem.FindData.cFileName-1);
      if(*pd.Owner) {
    ...
    *(ULONGLONG*)&pd.ftCreation = *(ULONGLONG*)&ftSystemTime - (ULONGLONG)pd.dwElapsedTime * 10000000;
      ...
      CurItem.FindData.ftCreationTime = CurItem.FindData.ftLastWriteTime =
    CurItem.FindData.ftLastAccessTime = pd.ftCreation;
...
void PrintModulesNT(FILE* InfoFile, DWORD dwPID, _Opt& Opt)
{
    ...
    do {
        ...
        SIZE_T sz = min(sizeof wszModuleName,
            (char*)Data.hModule + Data.dwSizeOfImage - Data.lpModuleBaseName);

        if(ReadProcessMemory(hProcess, Data.lpModuleFileName, wszModuleName, sz,0)) {
            len += fprintf(InfoFile, " %s", OemString(wszModuleName));

...
//--------------------------------------------------------------------------------
void Connection::sendrequest(const std::string &cmd, char *local, char *remote)
...
void Connection::sendrequestINT(const std::string &cmd, char *local, char *remote)
{  
	...
	if(Host.SendAllo)
	{
		if(cmd[0] != g_manager.opt.cmdAppe[0])
		{
			...
			Log(( "ALLO %I64u", v ));
			if(command(g_manager.opt.cmdAllo + boost::lexical_cast<std::string>(v)) != RPL_COMPLETE)
			{
				Log(( "!allo" ));
				return;
			}
	...
	if ( restart_point ) {
		if ( !ResumeSupport && cmd[0] != g_manager.opt.cmdAppe[0] ) {
			...
			if ( cmd[0] != g_manager.opt.cmdAppe[0] &&
				command(g_manager.opt.cmdRest_ + " " + boost::lexical_cast<std::string>(restart_point)) != RPL_CONTINUE)
				return;
	...
	case TYPE_A: if ( fsz != 0 ) {
		...
		if ( isPluginAvailable(FTP_NOTIFY_MAGIC) ) FTPNotify().Notify( &ni );
	...
	if ( isPluginAvailable(FTP_NOTIFY_MAGIC) ) {
		ni.Starting = FALSE;
		ni.Success  = TRUE;
		FTPNotify().Notify( &ni );
	}

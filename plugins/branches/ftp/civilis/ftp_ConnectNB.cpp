...
int Connection::nb_waitstate( SOCKET *peer, int state,DWORD tmW )
  {  fd_set           _readfds;
    ...
    GET_TIME( tmB );
    hStart = g_manager.opt.IdleStartPeriod ? FP_PeriodCreate(g_manager.opt.IdleStartPeriod) : NULL;
...
//Check full timeout
    ...
    if ( tmDiff > g_manager.opt.WaitTimeout ) {
Log(("global timeout finished %08X-%08X=%3.3f > %d",tmE,tmB,tmDiff,g_manager.opt.WaitTimeout ));
...
//Idle
    ...
    if ( g_manager.opt.WaitIdle && g_manager.opt.WaitCounter &&
         (!inIdle || CMP_TIME(tmE,idleB) > (1.0 / g_manager.opt.WaitCounter)) ) {
      Sleep(g_manager.opt.WaitIdle);
      idleB  = tmE;
      inIdle = true;
    }
...
//DISPATCH SELECT
    if ( res == 0 ) {
      ...
      if ( g_manager.opt.ShowIdle && hIdle && FP_PeriodEnd(hIdle) &&
        ...
        if ( brk_flag )
          ...
          buff.printf( "%s (%.1f%s): %2d%% ",
                       FP_GetMsg(MWaiting), tmDiff, FP_GetMsg(MSeconds), (int)(tmDiff*100/g_manager.opt.WaitTimeout)+1 );

        SetLastError( ERROR_SUCCESS );
        IdleMessage( buff.c_str(),g_manager.opt.IdleColor );
      }
    } else
//Write

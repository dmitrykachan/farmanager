...
// TYPES
  ...
  extern FTPPluginInterface* DECLSPEC FTPPluginGetInterface( void );
...
//Reg
   int      (DECLSPEC    *GetRegKeyFullInt)(const wchar_t* Key,CONSTSTR ValueName,DWORD Default );
   char*    (DECLSPEC    *GetRegKeyFullStr)(const wchar_t* Key,CONSTSTR ValueName,char *ValueData,CONSTSTR Default,DWORD DataMaxSize );
...
//Std
   ...
   char*    (DECLSPEC    *StrCpy)( char *dest,CONSTSTR src, size_t dest_sz /*=-1*/ );
   ...
   size_t   (DECLSPEC_PT *SNprintf)( char *Buff,size_t cn,CONSTSTR Fmt,... );
   int      (DECLSPEC    *VSprintf)( char *Buff,CONSTSTR Fmt,va_list arglist );
   size_t   (DECLSPEC    *VSNprintf)( char *Buff,size_t cn,CONSTSTR Fmt,va_list arglist );
...
//Utilities
   ...
   char*    (DECLSPEC    *FDigit)( char *buff,__int64 Value, size_t BuffSize /*-1*/ );  // Output digit to string. Delimits thousands.
...
//info
   OptionsPlugin* (DECLSPEC *GetOpt)( void );
   FTPHostPlugin* (DECLSPEC *GetHostOpt)( void );
};
...
//------------------------------------------------------------------------
#include "../Progress/fp_Progress.h"
#include "../Dirlist/fp_DirList.h"
#include "../Notify/fp_Notify.h"

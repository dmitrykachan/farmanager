...
extern std::map<unsigned int, std::wstring> g_stringmap;

inline unsigned int addString(const std::wstring& str)
{
	static unsigned int stringId = 0;
	g_stringmap.insert(std::make_pair(++stringId, str));
	return stringId;
}

inline void removeString(unsigned int id)
{
	g_stringmap.erase(id);
}

inline bool FPIL_ADDEXIST(const PluginPanelItem *p) 
{
	return p->Reserved[0] != 0;
}

inline int FPIL_ADDDATA(const PluginPanelItem *p) // TODO
{
	return p->Reserved[0];
}

inline const std::wstring FPIL_ADD_STRING(const PluginPanelItem *p) // TODO
{ 
	std::map<unsigned int, std::wstring>::const_iterator itr = 
		g_stringmap.find(p->Reserved[0]);
	if(itr != g_stringmap.end())
		return itr->second;
	else
		return L"";
}

inline void FPIL_ADDSET(PluginPanelItem *p, unsigned int id) 
{ 
	p->Reserved[0] = id;
}

inline void FPIL_ADDSET(PluginPanelItem *p, const std::wstring &str) 
{ 
	p->Reserved[0] = addString(str);
}

struct FP_ItemList
{
    PluginPanelItem *List;         ///< Panel items array
    size_t			ItemsCount;   ///< Number of items in array
  ...
  public:
    ...
    PluginPanelItem *Add( const PluginPanelItem *src, size_t cn );                         ///<Add a `cn` items to list
    PluginPanelItem *Add( const PluginPanelItem *src )  { return Add(src,1); }
	void             Free( void );                                                     //Clear list

    PluginPanelItem *Items( void )                      { return List; }
    size_t			Count( void )                      { return ItemsCount; }
    PluginPanelItem *Item(size_t num )                    { return (num >= 0 && num < ItemsCount) ? (List+num) : NULL; }
...
#if !defined(__FP_NOT_FUNCTIONS__)
#if !defined(__USE_165_HEADER__)
extern int DECLSPEC FP_GetColorDialog( int CurrentColor,FLngColorDialog* LngDescriptions,CONSTSTR Help );
#endif
#endif

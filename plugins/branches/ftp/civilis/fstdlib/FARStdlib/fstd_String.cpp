...
{  
	size_t sz = VSNprintf( NULL,0,fmt,list );
	Alloc( (int)sz+1 );
	return len = (int) VSNprintf( str,maxchar,fmt,list );
}

void String::BeginSet( size_t sz )
{
	str = (char*)_Alloc( maxchar = (int)std::max( (size_t)DEF_STR_ALLOC, sz ) );
	str[len = 0] = 0;
}

...
public:
	...
	HANDLE OpenPlugin(int nOpenFrom, int nItem);
	enum EDoMenu
	{
		DOMNU_OK,
		DOMENU_CANCELLED,
		...
		DOMNU_ERR_SHOW,
		DOMNU_ERR_INVOKE,
	} OpenPluginBkg(int nOpenFrom, int nItem);
	...
	int AdvControl(int nCommand, void *pParam);
	HINSTANCE m_hModule;
	IMalloc* m_pMalloc;
protected:
	...
	void ReadRegValues();
	static long WINAPI CfgDlgProcStatic(HANDLE hDlg, int Msg, int Param1, long Param2);
	void CfgDlgProc(HANDLE hDlg, int Msg, int Param1, long Param2);

...
{
	ptrdiff_t max_input = (from_end - from) & ~1;
	ptrdiff_t max_output = (to_limit - to);
	ptrdiff_t count = min(max_input/2, max_output);
...
{
	ptrdiff_t max_input = (from_end - from);
	ptrdiff_t max_output = (to_limit - to) & ~1;
	ptrdiff_t count = min(max_input, max_output/2);
					   ...
					   wchar_t* to, wchar_t* to_limit, wchar_t*& to_next) const
{
	...
	for(; to_next < to_limit && from_next < from_end; ++to_next) {
		...
		if (uchar(*from_next) < 0x80) *to_next = uchar(*from_next++);
			...
			if (from_end - from_next < static_cast<ptrdiff_t>(extra_bytes + 1)) 
				return partial;
				  ...
				  char* to, char* to_limit, char*& to_next) const
{
		...
		for (;from_next < from_end; ++from_next) {
			...
			if (symbol < 0x7F) {
				...
				if (to_limit - to_next >= static_cast<ptrdiff_t>(extra_bytes + 1)) {
